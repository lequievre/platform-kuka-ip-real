		...
		public:
		  ...
		  void printInterfaces(const std::list<hardware_interface::ControllerInfo> &start_list, const std::list<hardware_interface::ControllerInfo> &stop_list)
			...
			ControlStrategy getNewControlStrategy(const std::list< hardware_interface::ControllerInfo >& start_list, const std::list< hardware_interface::ControllerInfo >& stop_list, ControlStrategy default_control_strategy)
			{
				...
				for ( std::list<hardware_interface::ControllerInfo>::const_iterator it = start_list.begin(); it != start_list.end(); ++it )
				{
				    std::cout << "Control Strategy type = " << it->type << std::endl;
				    ...
				    {
				      std::cout << "InterfaceResources -> " << res_it->hardware_interface << std::endl;
				    }
					...
					else if( it->claimed_resources[0].hardware_interface == "hardware_interface::KUKAJointInterface" )
					{
					...
					else if( it->claimed_resources[0].hardware_interface == "hardware_interface::KUKACartesianInterface" )
					{
		  ...
		  {
				ROS_INFO("doSwitch ! lwr_hw_fril.hpp ");
				...
				ROS_INFO("doSwitch ! desired_strategy : %d", desired_strategy);
