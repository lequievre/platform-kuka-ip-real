...
namespace kuka_lwr_controllers 
{
    ...
    bool TorqueBasedPositionController::init(hardware_interface::KUKAJointInterface *robot, ros::NodeHandle &n)
    {
        ...
        sub_damping_ = n.subscribe("setDamping", 1, &TorqueBasedPositionController::setDamping, this);
        sub_stiffness_ = n.subscribe("setStiffness", 1, &TorqueBasedPositionController::setStiffness, this);
	...
	void TorqueBasedPositionController::setDamping(const std_msgs::Float64MultiArrayConstPtr& msg)
    {
		#if TRACE_Torque_Based_Position_ACTIVATED
			ROS_INFO("TorqueBasedPositionController: Start setDamping of robot %s!",robot_namespace_.c_str());
		#endif

		if(msg->data.size()!=joint_handles_.size())
		{ 
			ROS_ERROR_STREAM("TorqueBasedPositionController: setDamping Dimension (of robot " << robot_namespace_.c_str() << ") of command (" << msg->data.size() << ") does not match number of joints (" << joint_handles_.size() << ")! Not executing!");
			return; 
		}
	...
	void TorqueBasedPositionController::setStiffness(const std_msgs::Float64MultiArrayConstPtr& msg)
    {
		#if TRACE_Torque_Based_Position_ACTIVATED
			ROS_INFO("TorqueBasedPositionController: Start setStiffness of robot %s!",robot_namespace_.c_str());
		#endif

		if(msg->data.size()!=joint_handles_.size())
		{ 
			ROS_ERROR_STREAM("TorqueBasedPositionController: setStiffness Dimension (of robot " << robot_namespace_.c_str() << ") of command (" << msg->data.size() << ") does not match number of joints (" << joint_handles_.size() << ")! Not executing!");
			return; 
		}
